document.addEventListener('DOMContentLoaded', function() {
	...
	function randomQuestion() {
		...
		do {
			...
			question_word.innerHTML = list[index].word;
			stop = true;
		} while (!stop)
		...
		while (arrayAnswer.length < 3) {
			...
			if (index_answer != index) {
				if (!arrayAnswer.includes(list[index_answer].meaning.trim())) {
					arrayAnswer.push(list[index_answer].meaning.trim());
				}
			}
		}
		arrayAnswer.push(list[index].meaning.trim());
		arrayAnswer.sort();
		...
		arrayAnswer.forEach(function(item) {
			answer += `<div data='${item.trim()}' class="btn_answer">
		...
		btn_answer.forEach(function(button) {
			button.addEventListener('click', function() {
				...
				list.forEach(function(item) {
					if (item.word.trim() == question_word.textContent && !item.status) {
						if (item.meaning.trim() == data) {
							item.status = true;
